...
function createLnAddressStore() {
  ...
  return {
    ...
    // Set success state
    setSuccess: (lnurl: string, lnAddress: string, bip353Address?: string) =>
      update((state) => ({
        ...state,
        lnurl,
        lnAddress,
    ...
    // Set update error
    setUpdateError: (error: Error, errorMessage?: string) =>
      update((state) => ({
        ...state,
        updateStatus: {
...
export const username = derived(lnAddressStore, ($store) => {
...
export const domain = derived(lnAddressStore, ($store) => {
  if (!$store.lnAddress) return null;
  const parts = $store.lnAddress.split("@");
