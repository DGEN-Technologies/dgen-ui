...
function getBaseUrl(): string {
  ...
  if (env.PUBLIC_DGEN_URL) {
    let parsed: URL;
    try {
      parsed = new URL(env.PUBLIC_DGEN_URL);
    } catch {
      throw new Error("PUBLIC_DGEN_URL must be a valid URL");
    }
    const isLocalhost =
      parsed.hostname === "localhost" || parsed.hostname === "127.0.0.1";
    if (parsed.protocol !== "https:" && !isLocalhost) {
      throw new Error("PUBLIC_DGEN_URL must use https");
    }
...
export function getCacheStats(): { size: number; keys: string[] } {
  return {
    ...
    keys: import.meta.env.DEV ? Array.from(memoryCache.keys()) : [],
  };
}
