...
import { PUBLIC_DGEN_NETWORK, PUBLIC_DGEN_URL } from "$env/static/public";
...
// Production-ready environment variable handling for Netlify
// In browser: use relative URLs (empty string) for same-origin requests
// On server: NEVER use external URLs directly - always use proxy endpoints
const base = "";
...
/**
 * Convert relative image URLs to full backend URLs for production compatibility
 * Handles legacy relative paths like /api/public/hash.jpeg
 * Auto-upgrades to HTTPS when page is served over HTTPS to prevent mixed content warnings
 */
export const getImageUrl = (url: string | null | undefined): string | null => {
  ...
  // Get the backend URL and upgrade to HTTPS if current page is HTTPS (prevents mixed content)
  let backendUrl = PUBLIC_DGEN_URL;
  if (!backendUrl) {
    // If PUBLIC_DGEN_URL is not defined, return the relative URL as-is
    return url;
  }
  ...
  // For any other relative path, assume it's on the backend
  return url;
};
...
export const login = async (
  ...
  if (u.language) cookies.set("lang", u.language, { ...opts, httpOnly: false });
...
export const register = async (user, ip, cookies, loginRedirect, fetchFn = fetch) => {
  ...
  try {
    ...
    // Return the registration error immediately - don't try to login
    return svelteFail(400, { error });
  }
