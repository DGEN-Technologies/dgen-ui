...
import { auth, g, p, register, sats } from "$lib/utils";
import { error, redirect } from "@sveltejs/kit";

export async function load({ cookies, request, params, parent, fetch }) {
  ...
  try {
    ({ amount } = await p("/take", { amount, id }, fetch, auth(cookies)));
  } catch (e) {
