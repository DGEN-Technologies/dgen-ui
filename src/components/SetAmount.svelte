...
{#if settingAmount}
  ...
  >
    ...
    >
      ...
      <div class="overflow-y-auto flex-1 p-4 sm:p-8">
        <div class="space-y-4">
          ...
          <div class="text-center space-y-1 pb-4 border-b border-white/10">
            <h3
              class="text-lg sm:text-2xl font-bold text-white flex items-center justify-center gap-2 flex-wrap"
            >
              {#if invoiceType === "lightning"}
                <iconify-icon
            ...
            <p
              class="text-xs sm:text-sm text-white/60 flex items-center justify-center gap-1 flex-wrap"
            >
              {#if invoiceType === "lightning"}
                <iconify-icon icon="ph:clock-bold" width="12"></iconify-icon>
              {:else if invoiceType === "bitcoin"}
                <iconify-icon icon="ph:hourglass-bold" width="12"
                ></iconify-icon>
              {:else if invoiceType === "liquid"}
          ...
          {#if invoiceType === "bitcoin"}
            ...
            <div class="space-y-2">
              <p class="text-xs text-white/50 text-center px-2">
                Quick minimum amount:
              </p>
              <button
                type="button"
      ...
      <div
        class="flex-shrink-0 p-4 sm:p-6 border-t border-white/10 flex flex-col gap-2 sm:flex-row sm:gap-3 sm:items-center"
      >
        <button
          bind:this={submit}
          type="submit"
