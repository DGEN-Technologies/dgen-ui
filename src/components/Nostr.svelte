<!-- NOSTR COMPONENT COMMENTED OUT - NOT USED IN THIS APP -->
...
<!-- <script lang="ts">
  ...
  let nostrConnect = async () => {
    ...
    connectUrl = `nostrconnect://${pubkey}?relay=${encodeURIComponent(nostrConnectRelay)}&perms=sign_event%3A1&name=DGEN Wallet&secret=${connectionSecret}`;
  ...
  let signUrl = $derived(
    `nostrsigner:${encodeURIComponent(JSON.stringify($eventToSign))}?compressionType=none&returnType=signature&type=sign_event&appName=DGEN Wallet`,
  );
</script>
