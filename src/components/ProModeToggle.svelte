...
<div class="relative flex items-center">
  ...
  >
    <span
      class="text-sm font-medium {proModeValue
        ? 'text-cyan-400 font-bold drop-shadow-[0_0_10px_rgba(34,211,238,0.5)]'
        : 'text-white/80'}"
    >
      PRO MODE
    </span>
    <div class="flex items-center justify-center w-5 h-5">
      {#if proModeValue}
        <iconify-icon
          ...
          class="text-cyan-400 animate-pulse drop-shadow-[0_0_10px_rgba(34,211,238,0.5)]"
          width="20"
        ></iconify-icon>
      {:else}
        <iconify-icon icon="ph:lightning" class="text-white/40" width="20"
        ></iconify-icon>
      {/if}
    </div>
  </button>
  ...
  {#if showTooltip}
    ...
    >
      <div
        class="absolute -top-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-base-200 rotate-45 border-t border-l border-white/10"
      ></div>
