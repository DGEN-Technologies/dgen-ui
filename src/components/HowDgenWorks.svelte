  ...
  const steps = [
    {
      ...
      description: "Sign up with just a username (email is optional). Your account is ready instantly - no complicated setup required.",
    ...
    },
    {
      ...
      time: "",
      description: "Show QR code or wallet address/username to sender. Receive payments from anyone anywhere in the world.",
      mobileDescription: "Share QR code or address. Receive payments instantly.",
    ...
    },
    {
      ...
      title: "Send Money",
      time: "",
      description: "Scan QR codes or paste addresses. Send Bitcoin anywhere instantly.",
      mobileDescription: "Scan QR codes. Send Bitcoin instantly.",
    ...
    },
    {
      ...
      time: "",
      description: "Buy Bitcoin, manage multiple wallets, and explore advanced features as you learn.",
      mobileDescription: "Buy Bitcoin and explore advanced features.",
...
{#if mounted}
  <section id="howItWorks" class="relative py-8 sm:py-24" bind:this={sectionElement}>
              ...
              >
                ...
                <div class="relative order-2 lg:order-1">
                  ...
                  >
                    ...
                    <div class="relative mx-auto max-w-sm">
                      <!-- Phone frame -->
                      <div class="glass rounded-3xl p-2 border-2 border-white/20">
                        <div class="bg-gradient-dark rounded-2xl p-3 sm:p-4 min-h-[200px] sm:min-h-[280px] flex flex-col justify-center">
                          <!-- Phone status bar -->
                          <div class="flex justify-between items-center mb-4 text-xs opacity-60">
                            <span>9:41</span>
                            <div class="flex gap-1">
                              <div class="w-4 h-2 bg-white/60 rounded-sm"></div>
                              <div class="w-6 h-2 bg-green-400 rounded-sm"></div>
                            </div>
                          </div>
                          
                          <!-- Step content -->
                          <div class="text-center space-y-2 sm:space-y-3">
                            <div class="text-3xl sm:text-4xl mb-1 sm:mb-2">{step.visual}</div>
                            <h4 class="text-lg sm:text-xl font-bold text-white">{step.title}</h4>
                            <p class="text-sm sm:text-base text-white/70">{step.action}</p>
                            
                            <!-- Simple action visualization -->
                            {#if i === 0}
                              <div class="glass rounded-lg p-2 sm:p-3 mt-2 sm:mt-4">
                                <div class="flex items-center justify-center gap-2">
                                  <iconify-icon icon="ph:user-bold" width="16" class="text-purple-400"></iconify-icon>
                                  <span class="text-sm sm:text-base">Choose username</span>
                                </div>
                              </div>
                            {:else if i === 1}
                              <div class="glass rounded-lg p-2 sm:p-3 mt-2 sm:mt-4">
                                <div class="w-14 h-14 sm:w-20 sm:h-20 mx-auto bg-white rounded-lg p-2 flex items-center justify-center">
                                ...
                                <div class="flex flex-col items-center justify-center gap-1">
                                  <div class="flex items-center justify-center gap-2">
                                    <div class="flex flex-col items-center">
                                      <iconify-icon icon="ph:camera-bold" width="16" class="text-yellow-400"></iconify-icon>
                                      <span class="text-xs opacity-60 mt-1">(scan)</span>
                                    </div>
                                    <iconify-icon icon="ph:arrow-right-bold" width="12" class="text-white/60"></iconify-icon>
                                    <iconify-icon icon="ph:paper-plane-bold" width="16" class="text-green-400"></iconify-icon>
                                  </div>
                                </div>
                              </div>
                            {:else}
                              <div class="glass rounded-lg p-2 sm:p-3 mt-2 sm:mt-4">
                                <div class="flex flex-col items-center justify-center">
                                  <!-- Coin stack image -->
                                  <img src="/images/bitcoin.png" alt="Bitcoin" class="w-16 h-16 object-contain" />
                                  <div class="flex items-center justify-center gap-2 mt-1">
                                    <iconify-icon icon="ph:trend-up-bold" width="16" class="text-green-400"></iconify-icon>
                                  </div>
                                </div>
                              </div>
                            {/if}
                    ...
                    {#if step.time}
                      <div class="mt-6 flex justify-center gap-6">
                        <div class="text-center px-4">
                          <p class="text-lg sm:text-xl lg:text-2xl font-bold gradient-text leading-relaxed">
                            {step.time}
                          </p>
                        </div>
                      </div>
                ...
                <div class="space-y-3 sm:space-y-6 order-1 lg:order-2">
                  <div class="flex items-center gap-3 sm:gap-4">
                    <div class="text-4xl sm:text-6xl font-bold gradient-text opacity-50">
                      {step.number}
                    </div>
                    <div
                    ...
                    >
                      <iconify-icon icon={step.icon} class="text-white text-2xl sm:text-4xl"
                      ></iconify-icon>
                    </div>
                  </div>

                  <h3 class="text-xl sm:text-2xl md:text-4xl font-bold">{step.title}</h3>
