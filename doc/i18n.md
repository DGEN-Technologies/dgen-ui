# DGEN Wallet Internationalization

DGEN Wallet can be translated into other languages by editing [JSON](https://www.json.org/json-en.html) files. You do not need programming knowledge to do this, or to use these instructions. Here are instructions on how to translate DGEN Wallet:
...
This file simply maps each language to its name. It is used both to get a list of languages and to get the name of each language for the language selector. If a translation is not in this file, DGEN Wallet will not be able to see it.
...
In the translation files, sections roughly correspond to where the the text is in DGEN Wallet. This can be useful if you need context for some text. Here are some of these associations (`A.B` means section `B` is located inside `A`):
...
If you have the `grep` tool installed and know how to use a terminal, you can use it to find where a certain bit of text is being used. In the main folder (usually called `dgen-ui`), you can use the following command to find where `transactions.submit` is being used, for example:
...
```JSON
"p1": "<strong>DGEN Wallet</strong> is an <strong>open source</strong> bitcoin web wallet powered by Breez SDK, providing on-chain and Lightning payment capabilities.",
```

These apply a certain style to the text between them. For example, the tags above will make "DGEN Wallet" and "open source" bold, leaving the rest non-bold.
...
Clone the repository, then use the guided translation script (at `src/locales/guided_translation.py`) to update the translation files. You will need to pull before running, and commit/push your changes in order for them to be applied to DGEN Wallet.
