...
test.describe("WalletStore", () => {
  ...
  test("should provide reactive wallet state", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { walletStore, isWalletUnlocked, walletError } = await import(
        "/src/lib/stores/wallet"
      );
      ...
      // Subscribe to stores
      const unsubState = walletStore.subscribe((state) =>
        stateChanges.push(state),
      );
      const unsubUnlock = isWalletUnlocked.subscribe((unlocked) =>
        unlockChanges.push(unlocked),
      );
      const unsubError = walletError.subscribe((error) =>
        errorChanges.push(error),
      );
  ...
  test("should handle wallet operations with state updates", async ({
    page,
  }) => {
    const result = await page.evaluate(async () => {
  ...
  test("should clear error state", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { walletStore, walletError } = await import(
        "/src/lib/stores/wallet"
      );
