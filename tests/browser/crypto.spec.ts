...
test.describe("CryptoService", () => {
  ...
  test("should generate different encrypted values for same data", async ({
    page,
  }) => {
    const result = await page.evaluate(async () => {
      const { CryptoService } = await import("/src/lib/cryptoService");
      ...
      return {
        ...
        bothWork:
          JSON.stringify(await crypto.decrypt(encrypted1, password)) ===
            JSON.stringify(data) &&
          JSON.stringify(await crypto.decrypt(encrypted2, password)) ===
            JSON.stringify(data),
      };
    });
