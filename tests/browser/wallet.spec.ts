...
test.describe("WalletService", () => {
  ...
  test("should generate and validate mnemonic", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { generateMnemonic, validateMnemonic } = await import(
        "/src/lib/walletService"
      );
  ...
  test("should NOT store mnemonic in localStorage or sessionStorage", async ({
    page,
  }) => {
    await page.evaluate(async () => {
      const { generateMnemonic, saveMnemonic } = await import(
        "/src/lib/walletService"
      );
    ...
    const hasInsecureStorage = await page.evaluate(() => {
      ...
      const hasMnemonic = [...localKeys, ...sessionKeys].some(
        (key) =>
      ...
      const sessionValues = sessionKeys.map(
  ...
  test("should require password for wallet operations", async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { saveMnemonic, isWalletLocked } = await import(
        "/src/lib/walletService"
      );
